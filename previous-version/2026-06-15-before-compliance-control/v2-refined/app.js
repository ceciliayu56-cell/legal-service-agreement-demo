...
function renderContracts() {
  ...
  contractRows.innerHTML = filtered
    .map((contract) => {
      ...
      const moreMenu = `<select class="more-select" aria-label="更多操作" data-more-action="${contract.id}"><option>更多</option><option>查看详情</option><option>底线突破申请</option><option>添加协办人</option><option>转交处理</option><option>作废合同</option></select>`;
  ...
  contractRows.querySelectorAll("[data-more-action]").forEach((select) => {
    select.addEventListener("change", () => {
      const contract = contracts.find((item) => item.id === select.dataset.moreAction);
      if (!contract) return;
      if (select.value === "查看详情") {
        window.location.href = `./detail.html?id=${encodeURIComponent(contract.id)}`;
      }
      if (select.value === "底线突破申请") {
        window.location.href = `./flow-demo.html?page=a&contractId=${encodeURIComponent(contract.id)}&action=breakthrough`;
      }
      if (["添加协办人", "转交处理", "作废合同"].includes(select.value)) {
        alert(`${select.value}：原型阶段先保留入口，后续接入独立流程。`);
      }
      select.value = "更多";
    });
  });
